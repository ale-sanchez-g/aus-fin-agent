...
resource "aws_security_group" "ecs_api" {
  ...
  egress  { from_port = 0; to_port = 0; protocol = "-1"; cidr_blocks = ["0.0.0.0/0"] }
  tags = merge(var.tags, { Name = "${var.project}-${var.environment}-ecs-api-sg" })
}
...
resource "aws_security_group" "ecs_web" {
  ...
  egress  { from_port = 0; to_port = 0; protocol = "-1"; cidr_blocks = ["0.0.0.0/0"] }
  tags = merge(var.tags, { Name = "${var.project}-${var.environment}-ecs-web-sg" })
}
...
resource "aws_security_group" "rds" {
  ...
  vpc_id      = aws_vpc.main.id
  ingress { from_port = 5432; to_port = 5432; protocol = "tcp"; security_groups = [aws_security_group.ecs_api.id] }
  egress  { from_port = 0; to_port = 0; protocol = "-1"; cidr_blocks = ["0.0.0.0/0"] }
